...
void onReceive(int packetSize) {
  ...
  while (LoRa.available()) {
    ...
    // Publish received LoRa data to MQTT
    ...
    doc["deviceId"] = device_ids[i]; // Assuming first device ID for simplicity

    char jsonBuffer[512];
    serializeJson(doc, jsonBuffer);

    Serial.print("Publishing received LoRa data message: ");
    Serial.println(jsonBuffer);
    client.publish(topics_pub[i], jsonBuffer); // Publish to the first topic
