...
const unsigned long smsInterval = 1800000;  // 15 minutes in milliseconds
String smsRecipient = "+917328021033"; 
...
String checkDataBalance() {
  // Send USSD code to check data balance
  String ussdCode = "*123#";  // Example USSD code, replace with your provider's code
  SerialMon.print("Sending USSD code: ");
  SerialMon.println(ussdCode);

  // Send USSD command
  String response = modem.sendUSSD(ussdCode);

  SerialMon.print("USSD Response: ");
...
void sendSMS() {
  ...
  String modemInfo = modem.getModemInfo();
  String signalQuality = String(modem.getSignalQuality()); // Ensure it's a String
  String simStatus = String(modem.getSimStatus()); // Ensure it's a String
  // modem.enableGPS()
  String battery = String(modem.getBattPercent());
  // Construct the message
  String message = "Modem Info: " + modemInfo + "\n";
  message += "Signal Quality: " + signalQuality + "\n";
  message += "SIM Status: " + simStatus + "\n";
  message += "Battery Status : " + battery ; 
  // Print and send the message
  Serial.print("Sending SMS to ");
  Serial.println(smsRecipient);

  if (modem.sendSMS(smsRecipient, message)) {
...
void loop() {
  ...
  if (now - lastSMSTime >= smsInterval) {
    // Time to send SMS
    sendSMS();
    lastSMSTime = now;  // Update lastSMSTime to current time
  }
}
